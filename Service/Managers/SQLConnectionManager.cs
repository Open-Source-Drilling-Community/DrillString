...
namespace NORCE.Drilling.DrillString.Service.Managers
{
    ...
    public class SqlConnectionManager
    {
        ...
        public static readonly string DATABASE_FILENAME = "DrillString.db";
        ...
        // 1) to avoid loading the complete DrillString (heavy weight data) each time we only need contextual info on the data (light weight data)
        // 2) to keep control of the logic of inserting and selecting a light data in the database
        //    localized at the controller/manager level (storing DrillStringLight as a whole could induce database corruption issues)
        // If the light weight data concept is not implemented, the same contextual info can be retrieved directly from the DrillString
        private readonly static Dictionary<string, string[]> _tableStructureDict = new Dictionary<string, string[]>()
            {
                { "DrillStringComponentTable", new string[] {
                    "ID text primary key",
                    "MetaInfo text",
                    "Name text",
                    "Description text",
